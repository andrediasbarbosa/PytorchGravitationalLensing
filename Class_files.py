    ...
    def clines(self,ax=None,color='red',alpha=1.0,lt='-'):
        ...
        if (sizevs.size > 0):
            for j in range(len(p)):
                ...
                # plot the results!
                ax.plot((np.array(x1)-self.npix/2.)*no,(np.array(x2)-self.npix/2.)*no,lt,color=color,alpha=alpha)
...
class PSIEc(gen_lens):
    ...
    def potential(self):
        ...
        #
        self.pot = np.sqrt(((x - x0) * self.pixel) ** 2 / (1 - self.ell) + ((y - y0) * self.pixel) ** 2 * (1 - self.ell) + self.theta_c ** 2) * self.norm
